...
- `status` → read `.claude/loop/STATE.json` + `.claude/loop/verdict.json` and report status, iteration/cap, last failing gate, and the GOAL outcome. If `runBudget` is set, report the turns / output tokens spent against their caps. If `usageHold` is set, report it is waiting in-session for the usage window (window / % / resumeAt). If `status=paused`, report `pausedWindow` / `pausedUtil` / `resumeAt` and that `/goal-loop resume` continues it. Exit.
...
- **Budget** — `maxIterations`, `maxRepeatedFailures`, and the per-run ceiling `maxTurns` / `maxOutputTokens`.
...
**Run ceiling (escalation):** `maxIterations` only bounds how many times the Stop hook blocks, not the turns each block buys, so the hook also counts this session's turns and cumulative output tokens from the transcript (subagent transcripts included, deduped by `message.id`). At `budget.maxTurns` (default 3000) or `budget.maxOutputTokens` (default 1 000 000) it **allows the stop**, sets `status=budget_exhausted` with a `runBudget` record, writes `BLOCKER.md`, and tells the user the budget is spent **and the objective is unverified**. Never present that as done — report the last oracle state and what remains. See `references/config-keys.md`.
